...
optional_dependencies="alacritty zotero librewolf firefox nemo calibre zoom networkmanager yad mpv transmission-sequential-gtk"
soundpack="./soundpack/windowslonghorn"
soundpack_target="/usr/share/sounds/"
...
_optionalDependencyCheck() {
    echo -e "\n${BOLD}${BLUE}::${WHITE} Checking optional dependencies\n${GREEN}You may install these programs to enhance your experience${RESET}"
...
_installSoundpack() {
    echo -e "\n${BOLD}${BLUE}::${WHITE} Installing soundpack ${RESET}"
    if [ -e /usr/share/sounds/windowslonghorn ]; then
        echo -e "Soundpack already Installed ${RESET}"
        return 0
    else
        echo -e "Copying soundpack to '/usr/share/sounds'"
        ${superuser} cp -r ${soundpack} ${soundpack_target}
        echo -e "Soundpack installed ${RESET}"
        return 0
    fi
}
